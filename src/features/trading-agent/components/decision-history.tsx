...
export function DecisionHistory({ onBack }: Props) {
  ...
  if (loading) return (
    <div className="flex flex-col gap-4 p-5 rounded border border-zinc-900 bg-zinc-950/40 backdrop-blur-md">
  ...
  return (
    <div className="flex flex-col gap-4 p-5 rounded border border-zinc-900 bg-zinc-950/40 backdrop-blur-md relative overflow-hidden">
      ...
      <div className="flex flex-col gap-3 mb-4">
        <input 
          type="text"
          placeholder="Filter by symbol or status (e.g. 'blocked')..."
        ...
        <table className="w-full text-[11px] font-mono">
          <thead className="text-zinc-500 text-left border-b border-zinc-900/80 pb-2">
            <tr className="border-b border-zinc-900/80 pb-2">
              <th className="py-2 font-bold uppercase tracking-wider">Timestamp</th>
            ...
            {filteredHistory.map((h, idx) => {
              ...
              return (
                <tr key={h.id || idx} className="hover:bg-zinc-900/20 transition-all duration-150">
                  <td className="py-2.5 text-zinc-500">{timestamp.toLocaleTimeString()}</td>
                  ...
                  <td className="py-2.5 text-zinc-300">{h.decision.action} ({h.decision.strength > 0 ? '+' : ''}{h.decision.strength.toFixed(2)})</td>
                  <td 
                    className="py-2.5 text-zinc-500 max-w-[250px] truncate cursor-help"
